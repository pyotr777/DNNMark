...
for machine in clean_logs["machine"].unique():
    ...
    cwd = os.getcwd()
    logdir = cwd.split("DNNMark/")[1]
    g.fig.text(0, 0, logdir, ha="left", va="top", fontsize=12)
    ...
    if os.path.exists("README"):
        family = "monospace"
        with io.open("README", mode="r", encoding='utf-8') as f:
            text_ = f.read()
...
dnnmark_aggr = clean_logs.copy()
dnnmark_aggr = dnnmark_aggr.groupby(by=["machine", "back_algos", "algofwd", "batch"]).apply(group_func)
print "dnnmark_aggr shape:", dnnmark_aggr.shape
print dnnmark_aggr.head()
dnnmark_aggr = dnnmark_aggr[dnnmark_aggr["VGG time"] != 0]
print "dnnmark_aggr removed zeroes. New shape:", dnnmark_aggr.shape
dnnmark_aggr.reset_index(inplace=True)
dnnmark_aggr.drop(["tmp"], axis=1, inplace=True)
print "dnnmark_aggr reset index"
print dnnmark_aggr.head()
dnnmark_aggr = dnnmark_aggr[["machine", "back_algos", "algofwd", "batch", "VGG time"]]
dnnmark_aggr = dnnmark_aggr[dnnmark_aggr["VGG time"] != 0]

for machine in dnnmark_aggr["machine"].unique():
    mlogs = dnnmark_aggr[dnnmark_aggr["machine"] == machine]
    fg = sns.FacetGrid(mlogs, hue="algofwd", row="back_algos",
                       height=3, aspect=2, margin_titles=True, sharey=False)
    g = fg.map(plt.plot, "batch", "VGG time", lw=1, alpha=1, ms=4, marker="o",
               fillstyle="full", markerfacecolor="#ffffff").add_legend()
    plt.subplots_adjust(top=0.97)
    g.fig.suptitle("VGG-aggregated DNNMark time (s) on {}".format(machine), fontsize=18)
    ...
    # Read Keras logs
    keras = pd.read_csv(
        "/HDD2/ML/mlbenchmarks/Keras/keras/logs/mouse/microseries/keras2.2.4_cifar100_vgg_20190710/keras_Quadro_P2000_29.csv")
    print "KERAS"
    keras = keras[keras["machine"] == machine]
    print keras.head(2)
    ...
    for i, row in enumerate(axes):
        for j, column in enumerate(row):
            ...
            keras.plot("batch",
                       ...
                       label="keras")
            if j == 0:
                ax.set_ylabel("VGG time (s)", fontsize=12)
            if i == axes.shape[0] - 1:
                ax.set_xlabel("mini-batch size", fontsize=12)

    fig_file = "VGG_time_{}_wKeras.pdf".format(machine)
    plt.savefig(fig_file, bbox_inches="tight")
    print "Saved plot to", fig_file
    plt.close()
